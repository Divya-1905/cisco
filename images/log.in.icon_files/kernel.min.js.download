...
CQ_Analytics.Operator.DOESNOTCONTAIN="doesnotcontain";
...
addOperator(CQ_Analytics.Operator.DOESNOTCONTAIN,CQ.I18n.getMessage("does not contain",null,"Ex: language contains french, Ex: gender contains female"),function(s1,s2){if(s1){if(s2){s1=""+s1;
s2=""+s2;
return s1.toLowerCase().indexOf(s2.toLowerCase())==-1
}return true
}return false
...
}try{if(object&&object[property]){var toEval="";
...
CQ_Analytics.Utils.loadTeaserElementWithSpinner(p,g);
...
k=null
}$CQ("#"+g+"+ span.teaser-spinner").remove()
};
j.call();
...
if(!CQ_Analytics.CiscoProfileDataMgr){CQ_Analytics.CiscoProfileDataMgr=function(){this.addListener("beforepersist",function(){this.checkAuthorizableId()
},this)
};
CQ_Analytics.CiscoProfileDataMgr.prototype=new CQ_Analytics.PersistedSessionStore();
CQ_Analytics.CiscoProfileDataMgr.prototype.STOREKEY="PROFILEDATA";
CQ_Analytics.CiscoProfileDataMgr.prototype.STORENAME="ciscoprofile";
CQ_Analytics.CiscoProfileDataMgr.prototype.LOADER_PATH=CQ_Analytics.Utils.externalize("/apps/cq/personalization/components/ciscoprofileloader/content/load.js",true);
CQ_Analytics.CiscoProfileDataMgr.prototype.PROFILE_LOADER=CQ_Analytics.Utils.externalize("/apps/cq/personalization/components/ciscoprofileloader/content/load.json",true);
CQ_Analytics.CiscoProfileDataMgr.prototype.init=function(){var a=new CQ_Analytics.SessionPersistence();
var b=a.get(this.getStoreKey());
if(!b||b==""){this.data={};
for(var c in this.initProperty){this.data[c]=this.initProperty[c]
}}else{this.data=this.parse(b)
...
CQ_Analytics.CiscoProfileDataMgr.prototype.checkAuthorizableId=function(){if(!this.data){this.init()
}if(this.data.authorizableId){CQ_Analytics.CCM.setVisitorId(this.data.authorizableId)
}else{CQ_Analytics.CCM.setVisitorId("")
}};
CQ_Analytics.CiscoProfileDataMgr.prototype.getLabel=function(a){return a
};
CQ_Analytics.CiscoProfileDataMgr.prototype.getLink=function(a){return""
};
CQ_Analytics.CiscoProfileDataMgr.prototype.clear=function(){var a=new CQ_Analytics.SessionPersistence();
a.remove(this.getStoreKey());
this.data=null;
this.initProperty={}
};
CQ_Analytics.CiscoProfileDataMgr.prototype.getLoaderURL=function(){return CQ_Analytics.ClientContextMgr.getClientContextURL("/contextstores/ciscoprofiledata/loader.json")
};
CQ_Analytics.CiscoProfileDataMgr.prototype.loadProfile=function(authorizableId){var url=this.getLoaderURL();
url=CQ_Analytics.Utils.addParameter(url,"authorizableId",authorizableId);
try{var object=CQ.shared.HTTP.eval(url);
if(object){this.data={};
for(var p in object){this.data[p]=object[p]
...
CQ_Analytics.CiscoProfileDataMgr=new CQ_Analytics.CiscoProfileDataMgr();
CQ_Analytics.CCM.addListener("configloaded",function(){this.checkAuthorizableId();
this.addListener("update",function(b,d){var a="/bin/clientcontextdata";
var f={};
if(d&&d!="generatedThumbnail"){if(d=="operation"){if(this.getProperty(d).toUpperCase()=="RESET"){f[CQ.Sling.CHARSET]="utf-8";
f.operation="reset";
this.getSessionStore().reset();
var g=function(h,k,j){if(k){CQ.Util.reload(CQ.WCM.getContentWindow())
}};
CQ.HTTP.post(CQ.shared.HTTP.externalize("/bin/clientcontextdata"),g,f,this,true)
}else{if(this.getProperty(d).toUpperCase()=="SAVE"){f[CQ.Sling.CHARSET]="utf-8";
var c=this.getSessionStore().getData();
f.attributeName="accessLevel";
f.attributeValue="3";
for(var e in c){f["store_"+e]=c[e]
}var g=function(h,k,j){if(k){CQ.Util.reload(CQ.WCM.getContentWindow())
}};
CQ.HTTP.post(CQ.shared.HTTP.externalize("/bin/clientcontextdata"),g,f,this,true)
}}}}});
CQ_Analytics.CCM.register(this)
},CQ_Analytics.CiscoProfileDataMgr)
};